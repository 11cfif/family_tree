				...
				<div className='modal'>
					<div className='spousesModal'>
						...
						<ul>
							{linesComponents}
							<li>
								<button onClick={() => editClick(data.spouses[data.selectedId])}>Редактировань выделенного супруга(у)</button>
								<button onClick={() => changeClick(data.selectedId)}>Выбрать текущего супруга(у)</button>
